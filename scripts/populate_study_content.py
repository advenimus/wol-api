...
# Embedded StudyContentExtractor class
...
def populate_study_content():
    ...
    # Start with Genesis 1 for testing
    print("Extracting study content for Genesis 1...")
    ...
    if study_data:
        ...
        conn.commit()
        print(f"Successfully inserted study content for Genesis 1")
        print(f"- Outline items: {len(study_data['outline'])}")
        print(f"- Study articles: {len(study_data['study_articles'])}")
    else:
        print("Failed to extract study content")
    
    cur.close()
    conn.close()

if __name__ == "__main__":
    populate_study_content()